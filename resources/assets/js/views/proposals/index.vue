	...
	<div>
		<x-filterable :url="url" :sortable="sortable" :filter-group="filters"
			...
			<div slot="extra" class="float-right">
				<router-link to="/proposals/create" class="btn btn-primary w-xs">
					{{$t('new')}}
				</router-link>
			</div>
			<x-tr slot="heading">
			    <x-td type="th" size="1">{{$t('number')}}</x-td>
			    <x-td type="th" size="2">{{$t('issue_date')}}</x-td>
			    <x-td type="th" size="2">{{$t('expiry_date')}}</x-td>
			    <x-td type="th" size="4">{{$t('contact')}}</x-td>
			    <x-td type="th" size="1">{{$t('opportunity')}}</x-td>
			    <x-td type="th" size="2">{{$t('status')}}</x-td>
			</x-tr>
