    ...
    <div class="content-inner" v-if="show">
        <x-panel margin padding>
            ...
            <x-row line>
                ...
                <x-form-group :errors="errors.template_id" :label="$t('template')" col="3">
                    <x-tag-input :params="{type_id: '1'}" :value="form.template"
                                 @input="onTemplateChange" column="name" name="name"
                                 resource="templates">
                    </x-tag-input>
                </x-form-group>
                <x-form-group :errors="errors.opportunity_id" :label="$t('opportunity')" col="3" optional>
                    <x-tag-input :params="{contact_id: form.contact_id}" :value="form.opportunity" @input="value => { form.opportunity_id = value.id; form.opportunity = value }" column="number"
                                 name="number"
                                 removable
