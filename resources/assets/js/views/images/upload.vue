<template>
    <x-modal :footer="showFooter" :loading="isUploading" @cancel="cancel">
        ...
        <div class="upload">
            ...
            <div @dragover.prevent="onDragOver" @drop.prevent="onDrop" class="upload-inner" v-else>
                <div class="upload-icon">
                    <span class="icon icon-images"></span>
                </div>
                <p class="upload-text">{{$t('drag_and_drop')}}</p>
                <div class="upload-browse">
                    <x-button @click="onBrowse">{{$t('browse')}}</x-button>
                    <input @change="handleUpload" multiple ref="input" type="file" v-show="false">
                </div>
            </div>
        </div>
    </x-modal>
