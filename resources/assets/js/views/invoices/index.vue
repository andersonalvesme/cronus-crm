	...
	<div>
		<x-filterable :url="url" :sortable="sortable" :filter-group="filters"
			...
			<div slot="extra" class="float-right">
				<router-link to="/invoices/create" class="btn btn-primary w-xs">
					{{$t('new')}}
				</router-link>
			</div>
			<x-tr slot="heading">
			    ...
			    <x-td type="th" size="2">{{$t('due_date')}}</x-td>
			    <x-td type="th" size="3">{{$t('contact')}}</x-td>
			    <x-td type="th" size="2">{{$t('grand_total')}}</x-td>
