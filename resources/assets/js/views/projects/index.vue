	...
	<div>
		<x-filterable :url="url" :sortable="sortable" :filter-group="filters"
			...
			<div slot="extra" class="float-right">
				<router-link to="/projects/create" class="btn btn-primary w-xs">
					{{$t('new')}}
				</router-link>
			</div>
			<x-tr slot="heading">
			    <x-td type="th" size="1">{{$t('number')}}</x-td>
			    <x-td type="th" size="1">{{$t('start_date')}}</x-td>
			    <x-td type="th" size="1">{{$t('deadline_date')}}</x-td>
			    <x-td type="th" size="3">{{$t('contact')}}</x-td>
			    <x-td type="th" size="3">{{$t('title')}}</x-td>
