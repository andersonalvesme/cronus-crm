<template>
    <div class="content-inner" v-if="show">
        <x-panel margin padding>
            ...
            <x-row line>
                <x-form-group :errors="errors.contract_status_on_create_id" :label="$t('contract_status_on_create_id')"
                              col="4">
                    <x-tag-input :value="form.contract_status_on_create"
                                 @input="value => { form.contract_status_on_create_id = value.id; form.contract_status_on_create = value }"
                                 column="name"
                                 name="name"
                                 resource="contract_statuses">
                    </x-tag-input>
                </x-form-group>
                <x-form-group :errors="errors.contract_status_on_email_sent_id"
                              :label="$t('contract_status_on_email_sent_id')"
                              col="4">
                    <x-tag-input :value="form.contract_status_on_email_sent"
                ...
                <x-form-group :errors="errors.default_contract_template_id" :label="$t('default_contract_template_id')"
                              col="4">
                    <x-tag-input :params="{type_id: '2'}" :value="form.default_contract_template"
            ...
            <x-row>
                <x-form-group :errors="errors.default_contract_type_id" :label="$t('default_contract_type_id')" col="4">
                    <x-tag-input :params="{type_id: '2'}" :value="form.default_contract_type"
            ...
            <x-row line>
                <x-form-group :errors="errors.contract_email_subject" :label="$t('contract_email_subject')"
                              col="12"
                              v-model="form.contract_email_subject">
                </x-form-group>
            </x-row>
            <x-row line>
                <x-form-group :errors="errors.contract_email_template" :label="$t('contract_email_template')" col="12">
        ...
        <mini-crud title="contract_types"
                   url="settings/contract_types">
            <x-tr slot="heading">
        ...
        <mini-status-crud title="contract_statuses"
                          url="settings/contract_statuses">
            <x-tr slot="heading">
                <x-td size="1" type="th">{{$t('id')}}</x-td>
