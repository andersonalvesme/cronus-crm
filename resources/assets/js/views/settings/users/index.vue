	...
	<div>
		<x-filterable :exportable="false" :url="url" :sortable="sortable" :filter-group="filters"
			...
			<div slot="extra" class="float-right">
				<router-link to="/settings/users/create" class="btn btn-primary w-xs">
					{{$t('new')}}
				</router-link>
			</div>
			<x-tr slot="heading">
			    <x-td type="th" size="1">{{$t('id')}}</x-td>
			    <x-td type="th" size="3">{{$t('name')}}</x-td>
			    <x-td type="th" size="3">{{$t('email')}}</x-td>
			    <x-td type="th" size="2">{{$t('role')}}</x-td>
