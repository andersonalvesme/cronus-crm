		  	  ...
		  	  <x-tr>
		  	  	<template v-for="(col, cx) in theadActive">
		  	  		<td v-if="col.type === 'select'">
		  	  		  <select class="form-select" v-model="row[col.name]">
		  	  		    <option v-for="option in col.options">{{option}}</option>
		  	  		  </select>
		  	  		</td>
		  	  		<td v-if="col.type === 'text'">
		  	  		  <input type="text" class="form-control" v-model="row[col.name]">
		  	  		</td>
		  	  		...
		  	  		<td v-if="col.type === 'number'">
		  	  		  <input type="number" class="form-control" v-model="row[col.name]">
		  	  		</td>
		  	  		<td v-if="col.type === 'date'">
		  	  		  <input type="date" class="form-control" v-model="row[col.name]">
		  	  		</td>
		  	  		...
		  	  		<td v-if="col.type === 'textarea'">
		  	  		  <textarea class="form-control form-textarea" v-model="row[col.name]"></textarea>
		  	  		</td>
		  	  		<template v-if="col.type === 'hidden' && compute(col, row)">
		  	  		</template>
		  	  		<td v-if="col.type === 'computed_currency'">
		  	  			<span class="form-control">{{compute(col, row) | formatCurrency(field.currency)}}</span>
		  	  		</td>
		  	  	</template>
		  	  	<td>
			  	...
			  	<template v-for="(vl, vx) in tfootActive">
			  		...
			  		<tr v-if="vl.type === 'input_currency'">
			  			...
			  			<td :colspan="field.colspan.title" class="table-foot">
			  				<span class="table-td">
			  					<span>{{vl.title}}</span>
			  					<input type="text" class="form-control form-control-sm" v-model="vl.model">
			  				</span>
			  			</td>
			  			<td :colspan="field.colspan.value" class="table-foot">
			  		...
			  		<tr v-if="vl.type === 'tax'">
			  			...
			  			<td :colspan="field.colspan.title" class="table-foot">
			  				<span class="table-td">
			  					<span>{{vl.title}}</span>
			  					<input type="text" class="form-control form-control-sm" v-model="vl.percent_model"> %
			  				</span>
			  			</td>
			  			<td :colspan="field.colspan.value" class="table-foot">
