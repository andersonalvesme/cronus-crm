...
class ContractStatusController extends Controller
{
    ...
    public function store(Request $request)
    {
        $this->authorize('access', 'settings.contracts');
        $request->validate([
            'name' => 'required|unique:contract_statuses,name,NULL,NULL,tenant_id,' . auth()->user()->id,
            'color' => 'required|alpha_dash|unique:contract_statuses,color,NULL,NULL,tenant_id,' . auth()->user()->id,
    ...
    public function update($id, Request $request)
    {
        $this->authorize('access', 'settings.contracts');
        $request->validate([
            'name' => 'required|unique:contract_statuses,name,' . $id . ',id,tenant_id,' . auth()->user()->id,
            'color' => 'required|alpha_dash|unique:contract_statuses,color,' . $id . ',id,tenant_id,' . auth()->user()->id,
            'locked' => 'required|boolean'
        ]);
