...
class OpportunityStageController extends Controller
{
    ...
    public function store(Request $request)
    {
        $this->authorize('access', 'settings.opportunities');
        $request->validate([
            'name' => 'required|unique:opportunity_stages,name,NULL,NULL,tenant_id,' . auth()->user()->id,
            'color' => 'required|alpha_dash|unique:opportunity_stages,color,NULL,NULL,tenant_id,' . auth()->user()->id,
    ...
    public function update($id, Request $request)
    {
        $this->authorize('access', 'settings.opportunities');
        $request->validate([
            'name' => 'required|unique:opportunity_stages,name,' . $id . ',id,tenant_id,' . auth()->user()->id,
            'color' => 'required|alpha_dash|unique:opportunity_stages,color,' . $id . ',id,tenant_id,' . auth()->user()->id,
            'locked' => 'required|boolean'
        ]);
