...
class ProposalStatusController extends Controller
{
    ...
    public function store(Request $request)
    {
        $this->authorize('access', 'settings.proposals');
        $request->validate([
            'name' => 'required|unique:proposal_statuses,name,NULL,NULL,tenant_id,' . auth()->user()->id,
    ...
    public function update($id, Request $request)
    {
        $this->authorize('access', 'settings.proposals');
        $request->validate([
            'name' => 'required|unique:proposal_statuses,name,' . $id . ',id,tenant_id,' . auth()->user()->id,
            'color' => 'required|alpha_dash|unique:proposal_statuses,color,' . $id . ',id,tenant_id,' . auth()->user()->id,
            'locked' => 'required|boolean'
        ]);
