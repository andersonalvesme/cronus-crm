...
class OrganizationCategoryController extends Controller
{
    ...
    public function store(Request $request)
    {
        $this->authorize('access', 'settings.organizations');
        $request->validate([
            'name' => 'required|unique:organization_categories,name,NULL,NULL,tenant_id,' . auth()->user()->id,
        ]);
    ...
    public function update($id, Request $request)
    {
        $this->authorize('access', 'settings.organizations');
        $request->validate([
            'name' => 'required|unique:organization_categories,name,' . $id . ',id,tenant_id,' . auth()->user()->id,
        ]);
