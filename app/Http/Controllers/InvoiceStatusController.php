...
class InvoiceStatusController extends Controller
{
    ...
    public function store(Request $request)
    {
        $this->authorize('access', 'settings.invoices');
        $request->validate([
            'name' => 'required|unique:invoice_statuses,name,NULL,NULL,tenant_id,' . auth()->user()->id,
            'color' => 'required|alpha_dash|unique:invoice_statuses,color,NULL,NULL,tenant_id,' . auth()->user()->id,
    ...
    public function update($id, Request $request)
    {
        $this->authorize('access', 'settings.invoices');
        $request->validate([
            'name' => 'required|unique:invoice_statuses,name,' . $id . ',id,tenant_id,' . auth()->user()->id,
            'color' => 'required|alpha_dash|unique:invoice_statuses,color,' . $id . ',id,tenant_id,' . auth()->user()->id,
            'locked' => 'required|boolean'
        ]);
